...
func newGraphCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "graph renders the document-projection reference graph as a hand-rolled " +
			"Mermaid flowchart (default) or Graphviz DOT.\n\n" +
			"Nodes are colored by connected component; orphans and broken-link targets " +
			"are visually distinct. A large graph (>200 nodes) falls back to a focused " +
			"subgraph of orphans/broken + their neighborhood, with a truncation note. " +
			"Use --format dot for Graphviz, or --tree for the hierarchy variant.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			default:
				return exitCodeError{code: platform.ExitUsage,
					err: fmt.Errorf("invalid --format %q (want: mermaid, dot)", format)}
			}
			return writeOrStdout(cmd.Context(), cmd, cfg.OutputDir, name, content)
		},
	}
	cmd.Flags().StringVar(&format, "format", "mermaid", "diagram format: mermaid | dot")
