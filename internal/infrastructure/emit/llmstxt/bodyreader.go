...
// the reader independently re-verifies containment (the same EscapesRoot guard
// the scanner/writer use) so a hostile ID can never read outside root. A reader
// with an empty root reads nothing.
...
func (r *BodyReader) Read(id identity.DocumentID) ([]byte, error) {
	...
	// shared containment guard (the same one the scanner/writer use).
